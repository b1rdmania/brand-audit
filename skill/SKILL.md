...
### Platform & Directory Linking

**Every platform or directory mentioned in the report must be hyperlinked.** When writing findings, evidence items, action items, and fix package checklists, wrap platform names in `<a href="..." target="_blank">` links.

Common platform URLs:
- Trustpilot → `https://www.trustpilot.com`
- Clutch → `https://clutch.co`
- Sortlist → `https://www.sortlist.com`
- UpCity → `https://upcity.com`
- Design Rush → `https://www.designrush.com`
...
Don't link: Google Business, Shopify, Cargo, WordPress, Squarespace (tools they already use, not discovery platforms to sign up for).

Do NOT add links inside the `<pre class="prompt-pre">` Claude prompt block — that's plain text for copy-pasting.

### Competitor Linking

When competitors are mentioned in findings, actions, or the executive summary, link directly to their website. This helps the business owner see what they're up against without having to search.

For example: "Studios like <a href="https://www.pentagram.com" target="_blank">Pentagram</a> and <a href="https://www.studio-ashby.com" target="_blank">Studio Ashby</a> have strong Behance presence."

Only link competitors where you have a confirmed URL from discovery. Don't guess URLs.
